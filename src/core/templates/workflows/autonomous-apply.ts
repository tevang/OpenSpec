...
/**
 * Returns the reusable autonomous-apply skill.
 * The `instructions` field is the entire runnable workflow contract.
 */
export function getAutonomousApplySkillTemplate(): SkillTemplate {
  const skillName = 'openspec-autonomous-apply';
  const description =
    'Autonomous end-to-end OpenSpec change implementation: worktree isolation, bounded-concurrency apply, autonomous reference-run validation, local multi-agent review, interactive merge decision, and archive.';

  const instructions = `# Autonomous Apply Workflow — /openspec-autonomous-apply
...
3. After merge: run \`/opsx:archive <change-name>\`, remove worktree:
   \`\`\`bash
...
export function getAutonomousApplyCommandTemplate(): CommandTemplate | null {
  // Reserved for future slash-command generation; no-op for now.
  return null;
}
